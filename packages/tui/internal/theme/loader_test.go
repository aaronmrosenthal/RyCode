...
func TestLoadThemesFromJSON(t *testing.T) {
	...
	expectedThemes := []string{"tokyonight", "rycode", "everforest", "ayu"}
...
func TestLoadThemesFromDirectories(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(userConfig, "rycode", "themes"), 0755)
	os.MkdirAll(filepath.Join(projectRoot, ".rycode", "themes"), 0755)
	os.MkdirAll(filepath.Join(cwd, ".rycode", "themes"), 0755)
	...
	os.WriteFile(filepath.Join(userConfig, "rycode", "themes", "override-test.json"), []byte(testTheme1), 0644)
	os.WriteFile(filepath.Join(projectRoot, ".rycode", "themes", "override-test.json"), []byte(testTheme2), 0644)
	os.WriteFile(filepath.Join(cwd, ".rycode", "themes", "override-test.json"), []byte(testTheme3), 0644)
