...
// 2. USER_CONFIG/rycode/themes/*.json
// 3. PROJECT_ROOT/.rycode/themes/*.json
// 4. CWD/.rycode/themes/*.json
func LoadThemesFromDirectories(userConfig, projectRoot, cwd string) error {
	...
	dirs := []string{
		...
		filepath.Join(projectRoot, ".rycode", "themes"),
	}
	if cwd != projectRoot {
		dirs = append(dirs, filepath.Join(cwd, ".rycode", "themes"))
	}
